...
impl std::fmt::Display for Status {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Status::Downloading => write!(f, "Téléchargement"),
            Status::Installing => write!(f, "Installation"),
            Status::Configuring => write!(f, "Configuration"),
        }
    }
}
...
pub fn detect_distro() -> Distro {
    ...
    if let Ok(content) = std::fs::read_to_string("/etc/os-release") {
        ...
        if content_lower.contains("id=fedora") || content_lower.contains("id_like=fedora") {
...
fn categorize_package(name: &str) -> PackageCategory {
    ...
    if name_lower.contains("linux")
        ...
        || name_lower.contains("glibc")
        || name_lower.contains("coreutils")
    ...
    if name_lower.contains("steam")
        ...
        || name_lower.contains("gamemode")
        || name_lower.contains("mangohud")
    {
...
pub async fn install_system_updates(window: tauri::Window) -> Result<UpdateReport, String> {
    let distro = detect_distro();
...
async fn install_updates_apt(window: tauri::Window) -> Result<(Vec<String>, Vec<String>), String> {
    use tauri::Emitter;
        ...
        if line.contains("Unpacking") || line.contains("Setting up") {
            if let Some(pkg) = line.split_whitespace().nth(1) {
                let _ = window.emit(
                    "update-progress",
                    UpdateProgressEvent {
                        stage: if line.contains("Unpacking") {
                            Status::Installing
                        } else {
                            Status::Configuring
                        },
    ...
    if let Ok(running) = std::fs::read_to_string("/proc/version") {
        if let Ok(output) = Command::new("pacman").args(["-Q", "linux"]).output() {
            ...
            if !installed.is_empty()
                && !running.contains(installed.trim().split_whitespace().last().unwrap_or(""))
            {
                return true;
            }
        }
...
mod tests {
    ...
    fn test_detect_distro() {
        ...
        assert!(matches!(
            distro,
            Distro::Arch | Distro::SteamOS | Distro::Debian | Distro::Fedora | Distro::Unknown
        ));
    }
    ...
    fn test_categorize_package() {
        assert!(matches!(
            categorize_package("linux"),
            PackageCategory::System
        ));
        assert!(matches!(
            categorize_package("mesa"),
            PackageCategory::Graphics
        ));
        assert!(matches!(
            categorize_package("pipewire"),
            PackageCategory::Audio
        ));
        assert!(matches!(
            categorize_package("steam"),
            PackageCategory::Gaming
        ));
        assert!(matches!(
            categorize_package("libfoo"),
