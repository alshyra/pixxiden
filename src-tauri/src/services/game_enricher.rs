...
impl GameEnricher {
    ...
    pub fn new(config: EnricherConfig) -> Result<Self> {
        let cache = CacheManager::new().context("Failed to create cache manager")?;
    ...
    pub async fn enrich_game(&self, game: &Game) -> Result<EnrichedGame> {
        ...
        let is_stale = self
            .cache
            .is_metadata_stale(&game.id, self.config.cache_max_age_days)
            .await;
    ...
    async fn fetch_igdb_metadata(
        &self,
        title: &str,
    ) -> Option<crate::services::igdb::IGDBMetadata> {
    ...
    /// Fetch HLTB durations
    async fn fetch_hltb_durations(
        &self,
        title: &str,
    ) -> Option<crate::services::howlongtobeat::HLTBDurations> {
        match self.hltb.get_durations(title).await {
            Ok(Some(durations)) => {
                log::debug!(
                    "HLTB found durations for {}: main={:?}h",
                    title,
                    durations.main_story
                );
                Some(durations)
            }
            Ok(None) => {
    ...
    async fn fetch_protondb_compat(
        &self,
        steam_app_id: u32,
    ) -> Option<crate::services::protondb::ProtonDBCompatibility> {
        match self.protondb.get_compatibility(steam_app_id).await {
            Ok(Some(compat)) => {
                log::debug!(
                    "ProtonDB: {} tier for app {}",
                    compat.tier.as_deref().unwrap_or("unknown"),
                    steam_app_id
                );
                Some(compat)
            }
    ...
    async fn fetch_steam_achievements(
        ...
        match self.achievements.get_steam_achievements(steam_app_id).await {
            ...
            Err(e) => {
                log::warn!(
                    "Steam achievements fetch failed for {}: {}",
                    steam_app_id,
                    e
                );
                None
    ...
    async fn fetch_and_save_assets(&self, game_id: &str, title: &str) -> AssetPaths {
        ...
        if hero_exists {
            paths.hero = self
                .cache
                .get_existing_asset_path(game_id, AssetType::Hero)
                .await
        ...
        if grid_exists {
            paths.grid = self
                .cache
                .get_existing_asset_path(game_id, AssetType::Grid)
                .await
                .map(|p| p.to_string_lossy().to_string());
        }
        if logo_exists {
            paths.logo = self
                .cache
                .get_existing_asset_path(game_id, AssetType::Logo)
                .await
                .map(|p| p.to_string_lossy().to_string());
        }
        if icon_exists {
            paths.icon = self
                .cache
                .get_existing_asset_path(game_id, AssetType::Icon)
                .await
                .map(|p| p.to_string_lossy().to_string());
        }
        ...
        if !hero_exists {
            if let Some(hero) = assets.hero {
                if let Ok(path) = self
                    .download_and_save_asset(steamgriddb, game_id, &hero.url, AssetType::Hero)
                    .await
                {
        ...
        if !grid_exists {
            if let Some(grid) = assets.grid {
                if let Ok(path) = self
                    .download_and_save_asset(steamgriddb, game_id, &grid.url, AssetType::Grid)
                    .await
                {
        ...
        if !logo_exists {
            if let Some(logo) = assets.logo {
                if let Ok(path) = self
                    .download_and_save_asset(steamgriddb, game_id, &logo.url, AssetType::Logo)
                    .await
                {
        ...
        if !icon_exists {
            if let Some(icon) = assets.icon {
                if let Ok(path) = self
                    .download_and_save_asset(steamgriddb, game_id, &icon.url, AssetType::Icon)
                    .await
                {
    ...
    async fn download_and_save_asset(
        ...
        let (bytes, extension) = steamgriddb
            .download_image(url)
            .await
        ...
        let path = self
            .cache
            .save_asset(game_id, asset_type, &bytes, &extension)
            .await
    ...
    async fn build_enriched_game(&self, game: &Game, metadata: &GameMetadata) -> EnrichedGame {
        EnrichedGame {
            ...
            developer: metadata
                .developer
                .clone()
                .or_else(|| game.developer.clone()),
            publisher: metadata
                .publisher
                .clone()
                .or_else(|| game.publisher.clone()),
            genres: metadata.genres.clone(),
            release_date: metadata
                .release_date
                .clone()
                .or_else(|| game.release_date.clone()),
