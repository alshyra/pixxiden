...
impl AchievementsService {
    ...
    async fn get_steam_schema(&self, app_id: u32) -> Result<Vec<SteamAchievementSchema>> {
        let api_key = self.steam_api_key.as_ref()
    ...
    async fn get_steam_player_achievements(&self, app_id: u32) -> Result<Vec<SteamPlayerAchievement>> {
        let api_key = self.steam_api_key.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Steam API key not configured"))?;
        let steam_id = self.steam_id.as_ref()
    ...
    pub async fn get_steam_achievements(&self, app_id: u32) -> Result<Option<GameAchievements>> {
        ...
        let player_map: std::collections::HashMap<&str, &SteamPlayerAchievement> = player_achievements
            .iter()
    ...
    pub fn parse_legendary_achievements(output: &str) -> Option<GameAchievements> {
        ...
        for line in output.lines() {
            ...
            if in_achievements_section && (line.is_empty() || (line.contains(':') && !line.contains("Achievement"))) {
                if !line.is_empty() {
    ...
    fn parse_achievement_counts(line: &str) -> Option<(u32, u32)> {
        ...
        let parts: Vec<&str> = line.split(|c: char| !c.is_numeric()).filter(|s| !s.is_empty()).collect();
    ...
    pub async fn get_epic_achievements(&self, store_id: &str, legendary_path: &str) -> Result<Option<GameAchievements>> {
    ...
    pub async fn get_gog_achievements(&self, store_id: &str, gogdl_path: &str) -> Result<Option<GameAchievements>> {
...
"#;
    ...
    fn test_can_fetch_steam() {
        let service = AchievementsService::new(
...
mod integration_tests {
    ...
    async fn test_get_steam_achievements() {
        ...
        if let Some(achievements) = achievements {
            println!("Hollow Knight achievements: {}/{}", achievements.unlocked, achievements.total);
            ...
            println!("No achievements found (game may not have achievements or profile is private)");
        }
    }
