...
impl CacheManager {
    ...
    pub async fn save_game_metadata(&self, metadata: &GameMetadata) -> Result<()> {
        ...
        cache.games.insert(metadata.game_id.clone(), metadata.clone());
        self.save_metadata_cache(&cache).await?;
        Ok(())
    }
    ...
    pub async fn get_existing_asset_path(&self, game_id: &str, asset_type: AssetType) -> Option<PathBuf> {
        let game_dir = self.game_assets_dir(game_id);
    ...
    pub async fn save_asset(&self, game_id: &str, asset_type: AssetType, data: &[u8], extension: &str) -> Result<PathBuf> {
        let game_dir = self.game_assets_dir(game_id);
        ...
        log::debug!("Saved asset {:?} for game {} at {:?}", asset_type, game_id, path);
        Ok(path)
    }
    ...
    pub async fn delete_game_assets(&self, game_id: &str) -> Result<()> {
        ...
        if game_dir.exists() {
            fs::remove_dir_all(&game_dir).await
...
mod tests {
    ...
    async fn test_save_and_load_metadata() {
        ...
        assert_eq!(loaded.unwrap().description, Some("Test description".to_string()));
    }
    ...
    async fn test_save_and_check_asset() {
        ...
        let path = cache.save_asset("game123", AssetType::Hero, data, "jpg").await.unwrap();
    ...
    async fn test_delete_game_assets() {
        ...
        cache.save_asset("game456", AssetType::Hero, b"data", "jpg").await.unwrap();
        cache.save_asset("game456", AssetType::Grid, b"data", "png").await.unwrap();
    ...
    async fn test_clear_game_cache() {
        ...
        // Add asset
        cache.save_asset("clearthis", AssetType::Hero, b"data", "jpg").await.unwrap();
    ...
    fn test_sanitize_filename() {
        ...
        assert_eq!(sanitize_filename("special:chars*here"), "special_chars_here");
    }
}
