...
impl ApiKeysConfig {
    ...
    pub fn has_igdb(&self) -> bool {
        self.igdb_client_id.as_ref().map(|s| !s.is_empty()).unwrap_or(false)
            && self.igdb_client_secret.as_ref().map(|s| !s.is_empty()).unwrap_or(false)
    }
    
    /// Check if SteamGridDB is configured
    pub fn has_steamgriddb(&self) -> bool {
        self.steamgriddb_api_key.as_ref().map(|s| !s.is_empty()).unwrap_or(false)
    }
    
    /// Check if Steam API is configured
    pub fn has_steam(&self) -> bool {
        self.steam_api_key.as_ref().map(|s| !s.is_empty()).unwrap_or(false)
            && self.steam_id.as_ref().map(|s| !s.is_empty()).unwrap_or(false)
    }
    
    /// Check if any enrichment keys are configured
    pub fn has_any_enrichment(&self) -> bool {
        self.has_igdb() || self.has_steamgriddb()
    }
}
...
impl ApiKeysManager {
    ...
    pub fn load(&self) -> Result<ApiKeysConfig> {
        ...
        let config: ApiKeysConfig = serde_json::from_str(&content)
            .context("Failed to parse API keys config file")?;
        
        log::info!("Loaded API keys config from {:?}", self.config_path);
        log::info!(
            "  - SteamGridDB: {}",
            if config.has_steamgriddb() { "configured" } else { "not set" }
        );
        log::info!(
            "  - IGDB: {}",
            if config.has_igdb() { "configured" } else { "not set" }
        );
        log::info!(
            "  - Steam: {}",
            if config.has_steam() { "configured" } else { "not set" }
        );
    ...
    pub fn save(&self, config: &ApiKeysConfig) -> Result<()> {
        let content = serde_json::to_string_pretty(config)
            .context("Failed to serialize API keys config")?;
