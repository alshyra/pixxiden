...
impl GogdlAdapter {
    ...
    fn find_wine_ge(&self) -> anyhow::Result<PathBuf> {
        ...
        let potential_paths = vec![
            // Heroic's Wine-GE
            dirs::home_dir().map(|h| h.join(".config/heroic/tools/wine/wine-ge-latest/bin/wine")),
            // GE-Proton location
            dirs::home_dir()
        ...
        let games: Vec<Game> = installed_data
            ...
            .map(|(app_name, installed_game)| {
                ...
                let (wine_prefix, wine_version) = self
                    .read_heroic_game_config(&app_name)
...
impl StoreAdapter for GogdlAdapter {
    ...
    async fn list_games(&self) -> anyhow::Result<Vec<Game>> {
        ...
        let installed_data = self.read_installed_games();
        log::info!(
            "Installed GOG games from Heroic: {:?}",
            installed_data.keys().collect::<Vec<_>>()
        );
            ...
            .map(|arr| {
                ...
                arr.iter()
                    .filter_map(|g| {
                        ...
                        let install_path = installed_data
                            .get(app_name)
