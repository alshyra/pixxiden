<template>
    <div class="min-h-screen bg-gradient-to-br from-[#0f0f1a] via-[#1a1a2e] to-[#16213e] p-10">
        <div class="mb-10">
            <Button variant="outline" class="mb-6 group" @click="handleBack">
                <span class="text-xl font-bold mr-2 transition-transform group-hover:-translate-x-1">←</span>
                Retour
            </Button>
        ...
        <div v-if="authStore.loading" class="flex flex-col items-center justify-center py-16">
            <div class="w-12 h-12 border-4 border-purple-500/30 border-t-purple-500 rounded-full animate-spin mb-5">
            </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl">
            <StoreCard v-for="store in stores" :key="store" :store="store" :status="authStore.getStoreStatus(store)"
                :loading="currentAction === store" :is-focused="focusedStore === store" @connect="handleConnect"
        ...
        <Modal v-model="showLogoutConfirm" :title="'Confirmer la déconnexion'" size="sm">
