<template>
    <div class="rounded-2xl overflow-hidden transition-all duration-300" :class="[
        variantClasses,
        ...
        <div v-if="$slots.header || title" class="px-6 py-4 border-b border-white/5">
            <slot name="header">
                <div class="flex items-center justify-between">
                    <div>
                        <span v-if="label" class="text-[10px] font-black uppercase tracking-widest text-[#5e5ce6]">
                            {{ label }}
                        </span>
