<template>
    <div class="progress-bar">
        ...
        <div v-if="label || showValue" class="flex justify-between items-end mb-2">
            <span v-if="label" class="text-[9px] font-black uppercase tracking-widest" :class="labelColorClass">
                {{ label }}
            </span>
        ...
        <!-- Progress track -->
        <div class="w-full bg-white/5 rounded-full overflow-hidden" :class="[
        ...
        <div v-if="$slots.subtitle || (showSpeed && speed)" class="flex justify-between mt-1.5">
            <slot name="subtitle">
                <span v-if="showSpeed && speed" class="text-[8px] font-bold text-gray-500 uppercase tracking-tighter">
                    {{ speed }}
                </span>
            </slot>
        </div>
