<template>
  <Card variant="elevated" no-padding>
    <!-- Header avec cover et infos -->
    <div class="flex p-5 gap-5 border-b border-white/5 bg-white/2 items-center">
      <!-- Game Cover (click to customize images) -->
      <div
        class="group/cover w-24 h-24 shrink-0 bg-[#1a1a1e] rounded-lg overflow-hidden border border-white/10 shadow-xl relative cursor-pointer"
        title="Personnaliser les images"
        @click="$emit('customize')"
      >
        <img
          v-if="coverImage"
          :src="coverImage"
          :alt="game?.info.title"
          class="w-full h-full object-cover transition-all group-hover/cover:brightness-75"
        />
        <div
          v-else
          class="w-full h-full bg-gradient-to-br from-purple-600 via-blue-600 to-cyan-600 flex items-center justify-center transition-all group-hover/cover:brightness-75"
        >
          <span class="text-4xl">🎮</span>
        </div>
        <!-- Edit overlay on hover -->
        <div
          class="absolute inset-0 flex items-center justify-center opacity-0 group-hover/cover:opacity-100 transition-opacity bg-black/30"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-6 h-6 text-white drop-shadow-lg"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="1.5"
              d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"
            />
          </svg>
        </div>
      </div>

      <!-- Game Info -->
      <div class="flex-1 min-w-0">
        <h2
          data-testid="game-info-title"
          class="text-xl font-black italic tracking-tight text-white leading-tight truncate"
        >
          {{ game?.info.title?.toUpperCase() || "N/A" }}
        </h2>
        <p
          data-testid="game-info-developer"
          class="text-[9px] text-gray-500 font-bold uppercase tracking-widest"
        >
          {{ game?.info.developer || "N/A" }} • {{ releaseYear || "N/A" }}
        </p>

        <!-- Badges -->
        <div class="flex gap-2 mt-2 flex-wrap">
          <Badge v-if="game?.info.genres?.[0]" variant="muted">{{ game.info.genres[0] }}</Badge>
          <Badge v-if="game?.info.genres?.[1]" variant="muted">{{ game.info.genres[1] }}</Badge>
          <Badge v-if="game?.installation.cloudSaveSupport" variant="success"> ☁ Cloud Save </Badge>
        </div>
      </div>
    </div>

    <!-- Actions (Install/Play) - Smart Component autonome -->
    <div class="p-5 bg-gradient-to-b from-white/2 to-transparent">
      <GameActions />
    </div>

    <!-- Stats Grid -->
    <div class="grid grid-cols-3 text-center border-y border-white/5 bg-white/2">
      <div class="p-3 border-r border-white/5 flex flex-col items-center">
        <span class="text-[8px] font-black text-gray-600 uppercase mb-0.5">Temps</span>
        <span class="text-xs font-bold text-white">{{ formattedPlayTime }}</span>
      </div>
      <div class="p-3 border-r border-white/5 flex flex-col items-center">
        <span class="text-[8px] font-black text-gray-600 uppercase mb-0.5">Dernier</span>
        <span class="text-xs font-bold text-white">{{ formattedLastPlayed }}</span>
      </div>
      <div class="p-3 flex flex-col items-center">
        <span class="text-[8px] font-black text-gray-600 uppercase mb-0.5">Statut</span>
        <span class="text-xs font-bold text-orange-400 italic">{{ completionStatus }}</span>
      </div>
    </div>

    <!-- Achievements -->
    <div v-if="achievementsProgress" class="p-6">
      <ProgressBar
        :value="achievementsProgress.unlocked"
        :max="achievementsProgress.total"
        label="Succès"
        variant="accent"
        value-format="fraction"
        show-value
      />
    </div>
  </Card>
</template>

<script setup lang="ts">
import { Card, Badge, ProgressBar } from "@/components/ui";
import { useCurrentGame } from "@/composables/useCurrentGame";
import GameActions from "./GameActions.vue";

/**
 * GameInfoCard - Smart Component autonome
 * Carte principale avec infos du jeu
 * Récupère toutes ses données via useCurrentGame (store-based)
 * Emits customize event when cover image is clicked
 */

defineEmits<{
  customize: [];
}>();

const {
  game,
  coverImage,
  releaseYear,
  formattedPlayTime,
  formattedLastPlayed,
  completionStatus,
  achievementsProgress,
} = useCurrentGame();
</script>
