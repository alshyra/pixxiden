    ...
    <Card variant="elevated" no-padding>
        ...
        <div class="flex p-5 gap-5 border-b border-white/5 bg-white/2 items-center">
            ...
            <div class="w-24 h-24 shrink-0 bg-[#1a1a1e] rounded-lg overflow-hidden border border-white/10 shadow-xl">
                <img v-if="game?.coverUrl" :src="game.coverUrl" :alt="game?.title" class="w-full h-full object-cover" />
                <div v-else
        ...
        <div v-if="achievementsProgress" class="p-6">
            <ProgressBar :value="achievementsProgress.unlocked" :max="achievementsProgress.total" label="Succès"
                variant="accent" value-format="fraction" show-value />
        </div>
    </Card>
</template>
