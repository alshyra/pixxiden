<template>
    <Modal v-model="show" :title="is2FAStep ? 'Authentification à deux facteurs' : 'Connexion Amazon Games'" size="md">
        <div class="space-y-6">
            <!-- Login Form -->
            <div v-if="!is2FAStep && !success" class="space-y-4">
                <Input v-model="email" type="email" label="Email" placeholder="votre@email.com" :disabled="loading"
                    ref="emailInputRef" @keyup.enter="handleLogin" />

                <Input v-model="password" type="password" label="Mot de passe" placeholder="••••••••••"
                    :disabled="loading" :error="error || undefined" @keyup.enter="handleLogin" />
            </div>

            <!-- 2FA Input -->
            <div v-else-if="is2FAStep && !success" class="space-y-4">
                <p class="text-sm text-white/80 text-center leading-relaxed">
                    Entrez le code à 6 chiffres :
                </p>
                <Input v-model="twoFACode" type="text" placeholder="000000" maxlength="6" :disabled="loading"
                    :error="error || undefined" ref="twoFAInputRef" @keyup.enter="handle2FASubmit"
        ...
        <template #footer>
            <div class="flex justify-end gap-3">
                ...
                <Button v-if="!is2FAStep && !success" variant="primary" @click="handleLogin"
                    :disabled="!isFormValid || loading" :loading="loading">
                    Se connecter
                </Button>
                <Button v-else-if="is2FAStep && !success" variant="primary" @click="handle2FASubmit"
                    :disabled="!is2FACodeValid || loading" :loading="loading">
                    Valider
                </Button>
            </div>
        </template>
