...
async function parseYaml(filePath) {
  ...
  for (const line of lines) {
    ...
    if (line.match(/^    enabled:/)) {
      ...
      result.binaries[currentBinary].sources[currentPlatform].checksum =
      ...
      result.binaries[currentBinary].fallback.package = trimmed
        .split(":")[1]
        .trim()
        .replace(/"/g, "");
    }
...
async function downloadBinary(name, config, platform, rustTarget) {
  ...
  try {
    ...
    if (binaryConfig.fallback?.type === "python-wrapper") {
      console.log(
        `   Attempting fallback with Python wrapper for: ${binaryConfig.fallback.package}`,
      );
      return await createPythonWrapper(name, binaryConfig.fallback.package, rustTarget);
    }
...
async function main() {
  ...
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  console.log(
    `${colors.green}✅ Success: ${successCount}${colors.reset}  ${colors.red}❌ Failed: ${failedCount}${colors.reset}`,
  );
  console.log("");
