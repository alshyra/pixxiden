...
export async function ensureValidWindow(): Promise<boolean> {
  try {
    ...
    if (handles.length > 0) {
      ...
      console.log(`[ensureValidWindow] Switched to fallback window: ${handles[0].substring(0, 20)}`)
      return true
    }
 ...
 */
export async function waitForElements(selector: string, timeout = 5000): Promise<WebdriverIO.ElementArray> {
    ...
    async () => {
      ...
      for (const handle of handles) {
        try {
          ...
          console.log(`[waitForAppReady] Error checking window ${handle.substring(0, 20)}: ${(e as Error).message}`)
        }
      }
      return false
    },
