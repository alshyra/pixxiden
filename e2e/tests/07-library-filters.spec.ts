...
async function getVisibleGameCount(): Promise<number> {
  return await browser.execute(() => {
    ...
    const gameCards = document.querySelectorAll('[data-testid="game-card"], .game-card, [class*="GameCard"]')
...
describe('Library Filters', () => {
  before(async () => {
    ...
    await browser.waitUntil(
      async () => {
        ...
        const hasGameContent = bodyText.includes('Installé') || bodyText.includes('EPIC') || bodyText.includes('GOG')
  ...
  describe('Filter UI Rendering', () => {
    ...
    it('should not have any Vue warnings about undefined properties', async () => {
      ...
      const vuePropertyErrors = errors.filter(e => 
        e.msg.includes('Property') && e.msg.includes('not defined on instance')
      )
      
      console.log(`Vue property errors: ${vuePropertyErrors.length}`)
      if (vuePropertyErrors.length > 0) {
        console.log('Errors:', vuePropertyErrors.map(e => e.msg.substring(0, 200)))
      }
  ...
  describe('Filter Click Functionality', () => {
    ...
    it('should filter games when clicking "Epic" filter', async () => {
      ...
      const errors = await getTrackedErrors()
      const criticalErrors = errors.filter(e => 
        e.msg.includes('ReferenceError') || e.msg.includes('TypeError')
      )
