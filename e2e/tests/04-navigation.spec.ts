...
async function navigateToSettings() {
  ...
  const result = await verifyViewContent('SettingsView', ['CONFIGURATION', 'Système', 'Pixxiden', 'Comptes'], 100)
  ...
  if (!result.isRendered) {
    ...
    console.log(`[navigateToSettings] SettingsView rendered correctly with ${result.textLength} chars`)
  }
...
describe('Navigation', () => {
  ...
  describe('Settings Sections', () => {
    ...
    it('should show system section by default (French: Système)', async () => {
      ...
      const result = await verifyViewContent('System Section', ['Système', 'Linux', 'Kernel', 'Mémoire', 'Processeur'], 150)
      
      if (!result.isRendered) {
        console.log(`WARNING: System section may not have rendered. Found: ${result.foundTexts.join(', ')}`)
      }
    ...
    it('should show accounts section (French: Comptes)', async () => {
      ...
      if (exists) {
        ...
        if (!result.isRendered) {
          console.log(`WARNING: Accounts section may not have rendered. Found: ${result.foundTexts.join(', ')}`)
        }
      } else {
    ...
    it('should show advanced section (French: Avancé)', async () => {
      ...
      if (exists) {
        ...
        const result = await verifyViewContent('Advanced Section', ['Avancé', 'Proton', 'MangoHud'], 100)
        
        if (!result.isRendered) {
          console.log(`WARNING: Advanced section may not have rendered. Found: ${result.foundTexts.join(', ')}`)
        }
